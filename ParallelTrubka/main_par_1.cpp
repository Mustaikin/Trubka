...
#include <iostream>
#include <iomanip>
#include "CylinderPoissonSolver.h"
...
const int G_NY = 16;                 //*< кол-во ячеек вдоль оси R
	...
	for (int i = lo[0]; i <= hi[0]; i++) {
		for (int j = lo[1]; j <= hi[1]; j++) {
			for (int k = lo[2]; k <= hi[2] + 1; k++) {
				...
				//std::cout << "i=" << i << "j=" << j << "k=" << k << "\t" << QFz[i][j][k] << "\n";
...
void calc_pi()
{
	...
	for (int i = lo[0]; i <= hi[0]; i++) {
		for (int j = lo[1]; j <= hi[1]; j++) {
			for (int k = lo[2]; k <= hi[2]; k++) {
				double pres_S = S[i][j][k];
				S[i][j][k] = (ro_1[i][j][k] / TAU) * (du[i][j][k] + dv[i][j][k] + dw[i][j][k] - pres_S);
				//printf("%f\n",ro_1[i][j][k]);
				//S[i][j][k] = 0.0;
				//printf("%lf\n", S[i][j][k]);
			}
	...
	solveByFftBsor(pidin, S, TOTAL_NX, TOTAL_NY, TOTAL_NZ, DX[0], DX[1], DX[2], 0.0001);
